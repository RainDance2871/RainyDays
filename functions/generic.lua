...
function RainyDays.Is_even(card)
  return card:get_id() > 0 and card:get_id() <= 10 and card:get_id() % 2 == 0
end

function RainyDays.shakeit(juice_cards, tarot_pulse)
  play_sound('tarot1')
  
  for i = 1, #juice_cards do
    if juice_cards[i] then
      juice_cards[i]:juice_up(0.8, 0.5)
    end
  end
