SMODS.Consumable {
  ...
  loc_vars = function(self, info_queue, card)
    if G.GAME.last_constellation and G.GAME.last_constellation ~= 'c_RainyDays_gemini' then
    ...
    G.E_MANAGER:add_event(Event({
      ...
      func = function()
        if G.consumeables.config.card_limit > #G.consumeables.cards then
          ...
          G.GAME.last_constellation = 'c_RainyDays_gemini'
  ...
  can_use = function(self, card)
    if not G.GAME.last_constellation or  G.GAME.last_constellation == 'c_RainyDays_gemini' then 
      return false
    end
...
function set_consumeable_usage(card)
  ...
  if card.config.center.set == 'Constellation' and card.config.center_key ~= 'c_RainyDays_gemini'then
