...
extern MY_HIGHP_OR_MEDIUMP vec4 sphere_visible1;
extern MY_HIGHP_OR_MEDIUMP vec4 sphere_visible2;
extern MY_HIGHP_OR_MEDIUMP number sphere_visible2_extra;
extern MY_HIGHP_OR_MEDIUMP vec4 sphere_visible3;

extern MY_HIGHP_OR_MEDIUMP vec3 row_x;
extern MY_HIGHP_OR_MEDIUMP vec3 row_y;
extern MY_HIGHP_OR_MEDIUMP vec3 row_start;

extern MY_HIGHP_OR_MEDIUMP vec2 sphere_size;

float getSphereVisible(int i) {
    	if (i < 4) return sphere_visible1[i];
    	if (i < 8) return sphere_visible2[i - 4];
	if (i == 8) return sphere_visible2_extra;
    	if (i < 13) return sphere_visible3[i - 9];
	return 0.0;
}
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords) {
	...
	if (row < 0 || row > 2) {
		...
		int sphere = int(row_start[row] + floor((pixel.x - row_x[row]) /  sphere_size.x));
		tex.a *= getSphereVisible(sphere);
	}
...
vec4 position( mat4 transform_projection, vec4 vertex_position) {
   	if (hovering <= 0.) {
    	...
    	MY_HIGHP_OR_MEDIUMP float scale = 0.2*(-0.03 - 0.3*max(0., 0.3-mid_dist)) * hovering*(length(mouse_offset)*length(mouse_offset))/(2. -mid_dist);
