  ...
  calculate = function(self, card, context)
    if context.individual and context.cardarea == G.play then
      if SMODS.has_enhancement(context.other_card, 'm_bonus') or SMODS.has_enhancement(context.other_card, 'm_mult') then
        if not context.other_card.debuff and check_card_in_hand(context.other_card, context.full_hand) then
          ...
          if card_pos and context.full_hand[card_pos + 1] then
            context.full_hand[card_pos + 1].ability.perma_bonus = (context.full_hand[card_pos + 1].ability.perma_bonus or 0) + card.ability.extra.chip_bonus
            context.full_hand[card_pos + 1].ability.perma_mult = (context.full_hand[card_pos + 1].ability.perma_mult or 0) + card.ability.extra.mult_bonus
            return {
              message_card = context.full_hand[card_pos + 1],
