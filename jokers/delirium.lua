SMODS.Joker {
  ...
  add_to_deck = function(self, card, from_debuff)
    card.ability.extra.message_sent = G.GAME.rd_delirium_active or nil
  end,
  ...
  loc_vars = function(self, info_queue, card)
    local active = G.GAME.blind and G.GAME.blind.in_blind and G.GAME.rd_delirium_active
    return {
      vars = {
        colours = {
  ...
  calculate = function(self, card, context)
    if context.joker_main and G.GAME.rd_delirium_active then
      return {
        mult = card.ability.extra.mult_bonus
      }
    end
    
    if context.discard and G.GAME.rd_delirium_active and not card.ability.extra.message_sent then
