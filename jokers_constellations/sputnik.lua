SMODS.Joker {
  ...
  calculate = function(self, card, context)
    if context.before and not context.blueprint then
      card.ability.active = not hand_played(card, context.scoring_name)
      if card.ability.active then
        card.ability.played_hands[#card.ability.played_hands + 1] = context.scoring_name
      end
    end
    
    if card.ability.active and context.joker_main and #G.consumeables.cards + G.GAME.consumeable_buffer < G.consumeables.config.card_limit then
